...
public:
    ...
    const char* prepare_connect_message(const std::string& username);
    const char* prepare_list_message();
    const char* prepare_info_message(USER_ID_TYPE id);
    const char* prepare_send_message(USER_ID_TYPE id, const std::string& message);
    const char* prepare_recieve_message();

    const char* prepare_connack_message();
    const char* prepare_listreply_message(const std::vector<USER_ID_TYPE>& user_list);
    const char* prepare_inforeply_message(const std::string& username);
